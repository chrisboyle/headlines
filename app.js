...
function getArticles(url, limit, callback) {
...
function refreshHeadlines(callback) {
	var pending = Object.keys(FEEDS).length;
	var result = {};
	for (var f in FEEDS) {
		(function(feed) {
			getArticles(FEEDS[feed], ITEMS_PER_FEED, function(articles) {
...
function showHeadlines(out, result) {
	// TODO HTML encoding
...
http.createServer(function(request, response) {
	...
	refreshHeadlines(function(result){  // TODO cache
		showHeadlines(response, result);
	});
}).listen(8080);
